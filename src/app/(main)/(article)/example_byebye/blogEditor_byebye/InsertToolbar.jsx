...
const InsertToolbar = ({ onInsert, onShortcut }) => {
  ...
  const quoteTools = [
    { label: '따옴표', icon: 'quote' },
    { label: '버티컬 라인', icon: 'line' },
    { label: '말풍선', icon: 'bubble' },
    { label: '포스트잇', icon: 'postit' },
    { label: '프레임', icon: 'frame' },
  ];
  ...
  return (
    ...
    <div className="insert-toolbar">
      <div className="toolbar-left">
        {tools.map(tool => (
          <button key={tool.label} onClick={() => onInsert(tool.label)} title={tool.label}>
            <img src={`/image/article/icons/inserttoolbar/${tool.icon}.png`} alt={tool.label} />
          </button>
        ))}
      </div>
      {/* 우측 이미지 리스트 불러오는 매퍼 */}
